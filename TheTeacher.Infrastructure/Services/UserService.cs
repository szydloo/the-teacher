...
namespace TheTeacher.Infrastructure.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task RegisterAsync(Guid id, string email, string password, string username, string fullname, string role)
        {
            var user = await _userRepository.GetAsync(email);
            if(user != null)
            {
            ...
            await _userRepository.AddAsync(new User(id, email, hash, salt, username, fullname, role));
        }
